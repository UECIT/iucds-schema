 ...
 */
public class SXCMTSImpl extends uk.nhs.connect.iucds.ucr.impl.TSImpl implements uk.nhs.connect.iucds.ucr.SXCMTS
{
    ...
    public uk.nhs.connect.iucds.ucr.CsSetOperator xgetOperator()
    {
        synchronized (monitor())
        {
            ...
            target = (uk.nhs.connect.iucds.ucr.CsSetOperator)get_store().find_attribute_user(OPERATOR$0);
            if (target == null)
            {
                target = (uk.nhs.connect.iucds.ucr.CsSetOperator)get_default_attribute_value(OPERATOR$0);
            }
            return target;
        }
    }
    ...
    public void xsetOperator(uk.nhs.connect.iucds.ucr.CsSetOperator operator)
    {
        synchronized (monitor())
        {
            ...
            if (target == null)
            {
                target = (uk.nhs.connect.iucds.ucr.CsSetOperator)get_store().add_attribute_user(OPERATOR$0);
            }
            target.set(operator);
        }
    }
